...
def create_network(seq_length, optimizer, learning_rate, loss):
    # net = tflearn.input_data(shape=[None, seq_length, 1], name='input')
    # net = tflearn.lstm(net, 128, return_seq=True)
    # net = tflearn.lstm(net, 128)
    # net = tflearn.fully_connected(net, 2, activation='softmax')
    # net = tflearn.regression(net, optimizer=optimizer, learning_rate=learning_rate,
    #                          loss=loss, name='target')

    net = tflearn.input_data(shape=[None, seq_length], name='input')
    net = tflearn.embedding(net, input_dim=1000000, output_dim=128)
...
def test_model(x1_code, y1_label, x2_code, y2_label):
    ...
    network = create_network(seq_length, optimizer, learning_rate, loss)
    model = tflearn.DNN(network, tensorboard_verbose=0)
    model.fit(
        x_train, y_train,
        n_epoch=n_epoch, shuffle=True,
        validation_set=0.1,
        show_metric=True,
